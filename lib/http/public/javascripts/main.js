...
function init(state) {
  ...
  o('#retrySelected').click(function(){
    o('input:checked').each($(this).parent().find("a.retry").click);
  });

  o('#removeSelected').click(function(){
    o('input:checked').each(function () {
      $(this).parent().find("a.remove").click();
      $(this).parent().remove();
    });
  });

  o("#removeAllFailed").click(function () {
    request("jobs/failed/0..10000", function (jobs) {
  ...
  o("#retryAllFailed").click(function () {
    request("jobs/failed/0..10000", function (jobs) {
      $.each(jobs, function (i, job) {
        $.post("job", {type: job.type, data: job.data, priority: job.priority});
      });
    });
  });
