...
Worker.prototype.shutdown = function(fn){
  ...
  var self = this;
  setTimeout(function () {
    console.log("Job has still not completed. Setting state to inactive");
    self.job.state('inactive').save(function(err, res) {
      if(err){
        console.log("could not set the job state to inactive because of ", err);
        return fn();
      }
      console.log("Job state set to inactive");
      return fn();
    });
  }, this.job.data.killWaitTime || 30000);
};
