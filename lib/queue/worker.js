...
Worker.prototype.shutdown = function(fn){
  ...
  var self = this;
  setTimeout(function () {
    console.log("Job has still not completed. Setting state to failed");
    self.job.state('failed').save(function(err, res) {
      if(err){
        console.log("could not set the job state to failed because of ", err);
        return fn();
      }
      console.log("Job state set to failed");
      return fn();
    });
