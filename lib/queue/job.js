...
exports.get = function(id, fn){
  ...
  client.hgetall('q:job:' + job.id, function(err, hash){
    if (err) return fn(err);
    if (!hash) return fn(new Error('job "' + job.id + '" doesn\'t exist'));
    if (!hash.type) {
	    console.log("INVALID hash", hash);
	    return   fn(new Error('job "' + job.id + '" is invalid: ' + JSON.stringify(hash)));
    }
    ...
    job.is_indexed = hash.is_indexed;
    job.expires = hash.expires;
    job.expireOnFail = hash.expireOnFail;
    try {
...
function Job(type, data, options) {
  ...
  if ("undefined" != typeof options.expires && true == options.expires)
    this.expires = 'T';
  else
    this.expires = 'F';

  if ("undefined" != typeof options.expireOnFail && true == options.expireOnFail)
    this.expireOnFail = 'T';
  else
    this.expireOnFail = 'F';
}
...
Job.prototype.remove = function(fn){
  ...
  delJob = function() {
    ...
    if ('T' == this.expires)
      this.client.zrem('q:expiring:' + this.type, this.id);

    if ("undefined" != typeof client)
      client.removeAllListeners();
  }

  if ('T' === this.is_indexed)
    delJob.call(this);
  else {
    client = redis.pubsubClient();
    client.subscribe('q:indexing');
    client.on('message', function(channel, message) {
      message = JSON.parse(message);
      if (message.id == self.id && 'indexed' == message.event)
        delJob.call(self);
    });
  }
...
Job.prototype.state = function(state){
  ...
  if ('T' == this.expires && state != this._state && ('complete' == state || (state == 'failed' && this.expireOnFail == 'T'))) {
    client.zadd('q:expiring:' + this.type, Math.floor(new Date().getTime() / 1000), this.id)
  }
...
Job.prototype.update = function(fn){
  ...
  // set index state to prevent race aheads
  this.set('is_indexed', 'F');

  // set whether expires;
  this.set('expires', this.expires);

  this.set('expireOnFail', this.expireOnFail);
  ...
  try {
    ...
    if ("undefined" != typeof this.indexes) {
      var indexes = this.indexes.trim().split(/\s+/);
      searchKeys = {}

      if (indexes.length)
        for (var i = 0; i < indexes.length; ++i)
          if ("undefined" != typeof this.data[indexes[i]])
            searchKeys[i] = this.data[indexes[i]];

    } else {
      searchKeys = this.data;
    }
    searchKeys.type = this.type;
    searchKeys = JSON.stringify(searchKeys);

    json = searchKeys
  } catch (err) {}

  if(searchKeys && searchKeys.length && searchKeys.length !== 0){
    getSearch().index(json, self.id, function(err) {
      self.set('is_indexed', 'T');
      events.emit(self.id, 'indexed');
    });
  } else {
    self.set('is_indexed', 'T');
    events.emit(self.id, 'indexed');
