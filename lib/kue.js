...
Queue.prototype.shutdown = function(fn){
  var n = running = this.workers.length;
  
  // wrap fn to call it only once
  var origFn = fn, called = false;
  fn = function() {
    if (called) return;
    called = true;
  ...
  // shutdown workers one by one
  while (n--) {
    this.workers[n].shutdown(function(err) {
